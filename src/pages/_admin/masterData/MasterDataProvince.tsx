...
const Create = () => {
  ...
  useBackOnClose(`add-province-coverage-area`, open, onOpen, onClose);
  const { req } = useRequest({
    id: "crud-province-coverage-area",
  });
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      ...
      const url = `/api/piramid/admin/master-data/province-coverage-area`;
      const config = {
        url,
        method: "POST",
        data: payload,
        ...
        <DisclosureContent>
          <DisclosureHeader>
            <DisclosureHeaderContent
              title={`${l.add} ${l.master_data_navs.province}`}
            />
          </DisclosureHeader>

          <DisclosureBody>
            <FieldsetRoot>
              <form id="add_province_form" onSubmit={formik.handleSubmit}>
                <Field
                  label={l.name}
          ...
          <DisclosureFooter>
            <BackButton />
            <BButton colorPalette={"p"} type="submit" form="add_province_form">
              {l.add}
            </BButton>
          </DisclosureFooter>
        </DisclosureContent>
...
const TableData = (props: any) => {
  ...
  // Hooks
  const { l } = useLang();
  const dataId = useEditProvinceDisclosure((s) => s.data?.id);
  const { req, loading: deleteLoading } = useRequest({
    id: `crud-province-coverage-area-${dataId}`,
  });

  // Contexts
  const { rt, setRt } = useRenderTrigger();
  const setEditProvinceData = useEditProvinceDisclosure((s) => s.setData);
  const editProvinceOnOpen = useEditProvinceDisclosure((s) => s.onOpen);
  ...
  const rowOptions = [
    {
      ...
      callback: (rowData: any) => {
        setEditProvinceData(rowData?.originalData);
        editProvinceOnOpen();
      },
    },
    {
      ...
      confirmation: (rowData: any) => ({
        ...
        confirmCallback: () => {
          ...
          const url = `/api/piramid/admin/master-data/province-coverage-area/${rowData.originalData.id}/restore`;
          const config = {
            url,
            method: "POST",
          };
    ...
    },
    {
      ...
      confirmation: (rowData: any) => ({
        ...
        confirmCallback: () => {
          ...
          const url = `/api/piramid/admin/master-data/province-coverage-area/${rowData.originalData.id}`;
          const config = {
            url,
            method: "DELETE",
          };
...
const MasterDataProvince = () => {
  ...
  const dataState = useDataState({
    url: `/api/piramid/admin/master-data/province-coverage-area`,
    method: "GET",
    payload: {
...
export default MasterDataProvince;
